...
sys.path.append(str(Path(__file__).resolve() / '/cnvrg/open_model_zoo/demos/common/python'))
...
def build_argparser():
    ...
    args.add_argument("-v", "--vocab", help="Required. path to the vocabulary file with tokens",
                      required=False, default="/cnvrg/open_model_zoo/models/intel/bert-small-uncased-whole-word-masking-squad-int8-0002/vocab.txt", type=str)
    args.add_argument("-m", "--model", default=Path("/cnvrg/open_model_zoo/tools/downloader/intel/bert-large-uncased-whole-word-masking-squad-0001/FP16/bert-large-uncased-whole-word-masking-squad-0001.xml"),  help="Required. Path to an .xml file with a trained model",
...
def main():
    ...
    vocab = load_vocab_file("/cnvrg/open_model_zoo/models/intel/bert-small-uncased-whole-word-masking-squad-int8-0002/vocab.txt")
    log.info("{} tokens loaded".format(len(vocab)))

    # get context as a string (as we might need it's length for the sequence reshape)
    u = ["https://en.wikipedia.org/wiki/Bert_(Sesame_Street)"]
    paragraphs = get_paragraphs(u)
    context = '\n'.join(paragraphs)
    ...
    # read IR
    model_xml = Path("/cnvrg/intel/bert-large-uncased-whole-word-masking-squad-0001/FP16/bert-large-uncased-whole-word-masking-squad-0001.xml")
    ...
    if args.reshape:
        ...
        if seq < c:
            ...
            log.info("Skipping network reshaping, as (context length + max question length) exceeds the current (input) network sequence length")
